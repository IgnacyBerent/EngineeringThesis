...
from src.data_process.entropy.transfer_entropy_hist import transfer_entropy_hist
        ...
        for patient_id, cb_data_type, cb_data in self.iterate_cb_data():
            ...
            if both_directions:
                self._add_result(
                    cb_data_type=cb_data_type,
                    patient_id=patient_id,
                    field_name=f'te_dv_{x_name}->{y_name}',
                    value=transfer_entropy_dv(y, x),
                )

    def add_te_hist(self, x_name: str, y_name: str, both_directions: bool = True) -> None:
        for patient_id, cb_data_type, cb_data in self.iterate_cb_data():
            x = cb_data.get(x_name)
            y = cb_data.get(y_name)
            if x is None:
            ...
            self._add_result(
                ...
                value=transfer_entropy_hist(x, y),
            )
            if both_directions:
                self._add_result(
                    cb_data_type=cb_data_type,
                    patient_id=patient_id,
                    field_name=f'te_hist_{x_name}->{y_name}',
                    value=transfer_entropy_hist(y, x),
                )
