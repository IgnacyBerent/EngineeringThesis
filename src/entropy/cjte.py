...
def calculate_cjte(xi, y, xc, model_order_range=(4, 16)):
    ...
    - xc: np.array, the conditioning signal (normalized).
    ...
    def compute_prediction_error(y, predictors):
        ...
        predictors = np.column_stack(
            [predictors, np.ones(len(predictors))]
        )  # Add bias term
    ...
    def optimize_model_order(y, predictors, model_order_range):
        ...
        for order in range(model_order_range[0], model_order_range[1] + 1):
            ...
            y_ordered = y[order:]
            predictors_shifted = np.column_stack(
                [np.roll(predictors_ordered, -i, axis=0)[:-order] for i in range(order)]
            )
    ...
    predictors_full_shifted = np.column_stack(
        [
            np.roll(predictors_full, -i, axis=0)[:-model_order]
            for i in range(model_order)
        ]
    )
    predictors_restricted_shifted = np.column_stack(
        [
            np.roll(predictors_restricted, -i, axis=0)[:-model_order]
            for i in range(model_order)
        ]
    )
