...
The \textit{python} implementations of the alghorithms in this study were developed with usage of \textit{numpy} library, with \textit{pandas}, and \textit{matplotlib} for data read, write operations, and plotting (\cite{numpy};\cite{pandas};\cite{matplotlib}).
...
\begin{equation}\label{eq:ar-unidir}
  X_t = a_{11} \cdot X_{t-1} + \epsilon_{Xt}
\end{equation}
$$ Y_t = a_{22} \cdot Y_{t-1} + a_{21} \cdot X_{t-1} + \epsilon_{Yt} $$
...
\begin{equation}\label{eq:ar-bidir}
  X_t = a_{11} \cdot X_{t-1} + a_{12} \cdot Y_{t-1} + \epsilon_{Xt}
\end{equation}
$$ Y_t = a_{22} \cdot Y_{t-1} + a_{21} \cdot X_{t-1} + \epsilon_{Yt} $$

Experiment with different $a_{21}$ equal to $0, 0.1, 0.15, 0.2$ and $a_{11}=a_{22}=0.7$, $\epsilon = 0.3$, and $a_{12} = 0.15$ was conducted.

\subsection{Coupled Hénon Map models}
Coupled Hénon Map model was used to achive a nonlinear coupling from $X$ to $Y$.
It has a following mathematical representation:
\begin{equation}
  fx_t = 1.0 - a \cdot X_{t-1}^2 + b \cdot X_{t-2}
\end{equation}
$$ fy_t = 1.0 - a \cdot Y_{t-1}^2 + b \cdot Y_{t-2} $$
$$ X_t = fx $$
$$ Y_t = (1.0 - \gamma) \cdot fy_t + \gamma \cdot fx_t $$
$$ X_t = fx_t $$

To achive stable process $a=1.4$ and $0.3$ was set, and $\gamma$ parameter responsible for coupling strength was set to $0, 0.05, 0.1, 0.2$

\subsection{Trivariate AR(1) models}
To compare TE performance with CJTE where confounding variable is present the trivariate AR(1) model is the simplest approach.
This model is a extension of above described AR(1) models with the following formula:
\begin{equation}
  Z_t = a_z \cdot Z_{t-1} + \epsilon_{Zt}
\end{equation}
$$ X_t = a_x \cdot X_{t-1} + a_{xz} \cdot Z_{t-1} + \epsilon_{Xt} $$
$$ Y_t = a_y \cdot Y_{t-1} + a_{yx} \cdot X_{t-1} + a_{yz} \cdot Z_{t-1} + \epsilon_{Yt} $$

In this model the $a_x=0.6, a_y=0.6, a_z=0.6$ are standing for autoregression strength, coupling from $X$ to $Y$ is symbolized by $a_{yx}$, and the causality mediation from $Z$ to $X$ and $Y$ was achived by $a_{yz}=a_{xz}$ that was set on $0, 0.05, 0.1, 0.2$. 
The $\epsilon$ is standing for the noise of the corresponding signal like in equations \ref{eq:ar-unidir} and \ref{eq:ar-bidir}.
...
\begin{enumerate}
  ...
  \item \textit{nk.ppg\_clean()} - performs low-pass butterworth filtering (result in figure \ref{fig:nk-cleaned_signal})
...
The above equations correspond to the following code, and are ilustrated at figure \ref{fig:processing_signals}.
...
Processed signals like in the figure \ref{fig:processed_signals} can be easly then be used for TE calculations using the \textit{BaroreflexResultsGenerator} that just like classes above have abstracted methods for data processing for all the subjects.
\begin{minted}{python}
...
\begin{equation}
  CJTE_{(X,Y)\to Z|W}^{d} = \sum p(a_i) \cdot \text{log}(\frac{p(a_i)p(b_i)}{p(c_i)p(d_i)})
\end{equation}
where:
