...
\begin{itemize}
  ...
  \item \texttt{Pandas}: Employed for the manipulation and analysis of structured tabular datasets (\cite{pandas};\cite{pandas2}).
...
To validate correctness of proposed in this thesis TE implementation, appropiate benchmark with usage of synthetic signals was performed.
...
\begin{itemize}
  ...
  \item ETCO$_{2}$ recorded using a capnograph (NONIN)
\end{itemize}
...
Time or Index (i) on the X-axis of the plots were derived from the following property:
\begin{equation}
  \text{Time} = \frac{\text{Index}}{fs} \text{ } [s]
\end{equation}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.47\linewidth]{figs/materials-methods/physiological/experiment_setup.jpg}
...
\begin{figure}[H]
    ...
    \caption{An illustrative example of preprocessed raw signals for breathing at rest}
    \label{fig:raw-data}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{ABP signal filtered using \texttt{nk.ppg\_clean()}}
...
\begin{figure}[H]
    ...
    \caption{ABP signal with detected peaks obtained using \texttt{nk.ppg\_findpeaks()}}
...
\begin{figure}[H]
    ...
    \caption{Processing signals using \texttt{BaroreflexDataProcessor}}
...
\begin{figure}[H]
    ...
    \caption{Processed signals using \texttt{BaroreflexDataProcessor}}
...
\begin{figure}[H]
    ...
    \caption{Initial signals used to present TE implementation}
...
The significance level ($\alpha$) \texttt{alpha} to pass this uniformity test is by default set as $0.05$, what is proposed in the original paper, but the implementation proposed in this thesis allows for modification of this value \cite{dvp}.
