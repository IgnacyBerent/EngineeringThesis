...
\begin{equation}\label{eq:te}
  TE_{Y\to X}^{d} = \sum p(x_t,x_{t-1}^{(d)},y_{t-1}^{(d)}) \cdot log(\frac{p(x_t|x_{t-1}^{(d)},y_{t-1}^{(d)})}{p(x_t|x_{t-1}^{(d)})})
\end{equation}
...
\begin{equation}\label{eq:te-simplified}
  TE_{Y\to X}^{d} = \sum p(x_t,x_{t-1}^{(d)},y_{t-1}^{(d)}) \cdot log(\frac{p(x_t,x_{t-1}^{(d)},y_{t-1}^{(d)})p(x_{t-1}^{(d)})}{p(x_t,x_{t-1}^{(d)})p(x_{t-1}^{(d)},y_{t-1}^{(d)})})
\end{equation}
...
\begin{equation}\label{eq:cte}
  CTE_{Y\to X|Z}^{d} = \sum p(x_t,x_{t-1}^{(d)},y_{t-1}^{(l)},z_{t-1}^{(d)}) \cdot log(\frac{p(x_t|x_{t-1}^{(d)},y_{t-1}^{(d)},z_{t-1}^{(d)})}{p(x_t|x_{t-1}^{(d)},z_{t-1}^{(d)})})
\end{equation}
...
\begin{equation}\label{eq:jte}
  JTE_{(X,Y)\to Z}^{d} = \sum p(z_t,z_{t-1}^{(d)},x_{t-1}^{(l)},y_{t-1}^{(d)}) \cdot log(\frac{p(z_t|z_{t-1}^{(d)},x_{t-1}^{(d)},y_{t-1}^{(d)})}{p(z_t|z_{t-1}^{(d)})})
\end{equation}

\begin{equation}\label{eq:cjte}
  CJTE_{(X,Y)\to Z|W}^{d} = \sum p(z_t,z_{t-1}^{(d)},x_{t-1}^{(d)},y_{t-1}^{(d)},w_{t-1}^{(d)}) \cdot log(\frac{p(z_t|z_{t-1}^{(d)},x_{t-1}^{(d)},y_{t-1}^{(d)},w_{t-1}^{(d)})}{p(z_t|z_{t-1}^{(d)},w_{t-1}^{(d)})})
\end{equation}
...
For this paper I have decided to implement from scratch the Darbelly-Vajda Partitioning (DVP) algorithm for its reliability, and parameter free approach (implementation in section \ref{sec:te_dvp-implementation})  (\cite{lee2012transfer};\cite{rozo2021benchmarking};\cite{dvp}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figs/theoretical-fundations/dvp.png}
    \caption{DVP visualization for noise, implemented from scratch}
