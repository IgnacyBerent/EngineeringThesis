...
%TODO: Add equatuion for jte and use x1 x2 instead?
There are scenarios where accessing the information transfer from two sources is needed.
In such cases Joint Transfer Entropy (JTE) is needed, or to include also conditioning, Conditional Joint Transfer Entropy (CJTE).
CJTE allows to calculate transfer from two sources $X$ and $Y$ to $Z$, under condition $W$, yet usage of the same signal as both source and condition is also common \cite{porta2015disentangling}.
