...
Further studies sought to asses the BRS in a non-invasive way using classical time series analysis techniques from ABP (SAP) and RR (HP) recordings.
...
Studies suggest that these methods are equivalent to each other, under gaussian assumptions, however in case of non-linear dependencies, TE is found to be superior.
...
All of the following TE algorithms were implemented from scratch in \textit{python} programing language, with detailed description in section \ref{sec:te_dvp-implementation}.
...
In the literature symbol for the embedding dimension for the other signal is often represented by $l$, but my implementation assumes that $k=l$, what is not an uncommon practice. 
My notation will use symbol $d$ for embedding dimensions (\cite{Schreiber2000};\cite{lee2012transfer}).

Finally $TE_{Y\to X}$ from random process $Y$ to random process $X$, with the corenspoding states $y_t$, $x_t$ and embedding dimension $d$ is defined as:
...
\begin{equation}\label{eq:cte}
  CTE_{Y\to X|Z}^{d} = \sum p(x_t,x_{t-1}^{(d)},y_{t-1}^{(l)},z_{t-1}^{(d)}) \cdot \text{log}(\frac{p(x_t|x_{t-1}^{(d)},y_{t-1}^{(d)},z_{t-1}^{(d)})}{p(x_t|x_{t-1}^{(d)},z_{t-1}^{(d)})})
\end{equation}
...
\begin{equation}\label{eq:jte}
  JTE_{(X,Y)\to Z}^{d} = \sum p(z_t,z_{t-1}^{(d)},x_{t-1}^{(l)},y_{t-1}^{(d)}) \cdot \text{log}(\frac{p(z_t|z_{t-1}^{(d)},x_{t-1}^{(d)},y_{t-1}^{(d)})}{p(z_t|z_{t-1}^{(d)})})
\end{equation}
